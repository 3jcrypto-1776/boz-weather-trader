...
tests/                   → 1486 backend + 254 frontend = 1740 tests
  ...
  ├── trading/     → Trading engine: EV calc + guardrails, Kelly sizing, risk, cooldowns, queue, executor, scheduler, safety, sync, retraining trigger, bracket cap, resting order sync (339)
  ├── backtesting/ → Backtesting engine: schemas, risk sim, data loader, engine, metrics, integration (95)
  ├── api/         → API endpoints: auth, dashboard, dashboard stats, health, markets, queue, settings, trades, trades/sync, accuracy, optimization, calendar, version, update, training (159)
